        ...
        case TIOCGWINSZ:
            {
                ...
                rt_size_t rn;
                struct rt_serial_device *dummy_device;
                rt_device_t old_dev;
                
                /* create dummy device */
                dummy_device = RT_KERNEL_MALLOC(sizeof(struct rt_serial_device));
                if(dummy_device == RT_NULL)
                {
                    break;
                }
                
                rt_memcpy(dummy_device, (struct rt_serial_device *)rt_console_get_device(), sizeof(struct rt_serial_device));
                dummy_device->parent.read  = RT_NULL;
                dummy_device->parent.write = RT_NULL;
                
                ret = rt_device_register(&dummy_device->parent, "dummy1", dummy_device->parent.flag);
                if(ret != RT_EOK)
                {
                    break;
                }
                old_dev = rt_console_set_device("dummy1");

                p_winsize = (struct winsize*)args;
                rt_memset(_tio_buf,0,_TIO_BUFLEN); /* clear buffer */
              
                rt_device_open(old_dev, old_dev->flag);
                old_flag = old_dev->open_flag;
                old_dev->open_flag |= RT_DEVICE_FLAG_STREAM;
                /* sending the command to let terminal report the current area of text */
                rt_device_write(old_dev,0,"\x1b[18t",rt_strlen("\x1b[18t"));
                rt_thread_delay(100);
                /* receiving the report from terminal  */
                rn = rt_device_read(old_dev, 0, _tio_buf, _TIO_BUFLEN);
                old_dev->open_flag = old_flag;
                rt_device_close(old_dev);
                
                /* delete dummy device */
                rt_console_set_device(old_dev->parent.name);
                rt_device_unregister(&dummy_device->parent);
                RT_KERNEL_FREE(dummy_device);
